...
//		wp_enqueue_style($this->plugin_name, 'wp-color-picker', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		 ...
		 * class.
		 */
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'assets/js/admin.js', array( 'wp-color-picker' ), $this->version, false );
	}
	...
	function Note_Press_sticky_dashboard($post, $callback_args)
	{
		...
		{
			case 0: $icon = "<img src=".plugins_url('admin/images/P0.png', dirname(__FILE__))." alt='Icon not found'>";
					break;
			case 1: $icon = "<img src=".plugins_url('admin/images/P1.png', dirname(__FILE__))." alt='Icon not found'>";
					break;
			case 2: $icon = "<img src=".plugins_url('admin/images/P2.png', dirname(__FILE__))." alt='Icon not found'>";
					break;
		}		
	...
	function Note_Press_add_dashboard_widgets() 
	{
		...
		foreach ($stickylist as $sticky)
		{
			...
			{
				add_meta_box(
				'Note_Press-'.$sticky->ID.'-dashboard_widget',
				$sticky->Title,
		...
		switch ($menulevel)
		{
			case 0: $setting = 'edit_posts';break;
			case 1: $setting = 'publish_posts';break;
			case 2: $setting = 'moderate_comments';break;
			case 3: $setting = 'manage_options';break;
		}
		if (current_user_can($setting))
		{
			add_meta_box(
			'Note_Press_dashboard_widget',
	...
	function Note_Press_add_plugin_admin_menu() {
		...
		add_menu_page('Note Press', 'Note Press', $setting, 'Note_Press-Main-Menu', array($this, 'Note_Press_load_menu'),  plugins_url('admin/images/Note_Pressicon.png', dirname(__FILE__)));
		add_submenu_page('Note_Press-Main-Menu', __('Note Press Settings','Note Press'), __('Note Press Settings','Note Press'), 'manage_options', 'Note-Press-Settings', array($this, 'Note_Press_load_settings'));
	}
